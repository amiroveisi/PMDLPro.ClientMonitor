...
using PSYCOPMDLProCoreCommunications.Web.Proxy.Compression;
using PSYCOPMDLProCoreCommunications.Web.Proxy.Helpers;
using PSYCOPMDLProCoreCommunications.Web.Proxy.Http;
using PSYCOPMDLProCoreCommunications.Web.Proxy.Http.Responses;
        ...
        private async Task copyBodyAsync(bool isRequest, bool useOriginalHeaderValues, HttpWriter writer, TransformationMode transformation, Action<byte[], int, int> onCopy, CancellationToken cancellationToken)
        {
            ...
            try
            {
                using (var bufStream = new CustomBufferedStream(s, BufferPool, BufferSize, true))
                {
                    ...
                    await writer.CopyBodyAsync(bufStream, false, contentLength, onCopy, cancellationToken);
                }
            }
            finally
            {
